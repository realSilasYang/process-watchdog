...
class HelpWindow extends ManagedWindow {
    ...
    BuildText() {
        ...
        lines := [
            ...
            Tr("• 选择文件夹会批量导入其中支持的文件；是否扫描子目录由“设置”中的“监控”控制。"),
            ...
            Tr("• 常规迁移不使用文件名、文件 ID 或目录监听作为判断依据。版本目录升级是受限例外：升级期间仅在同一父目录中存在唯一同名新版本入口时提出迁移，并记录、持续校验候选 SHA-256。发现多个候选、多个内容相同的副本或扫描未完整完成时不会猜测目标；确认后只更新守护路径，名称、图标和启动设置保持不变。"),
            ...
            "",
            Tr("五、设置"),
            Tr("• 显示：界面语言、内容字体和主题保存后会立即更新主窗口、菜单和托盘，无需重新启动。"),
            Tr("• 启动 / 监控：设置状态检查间隔、重启等待序列、启动后是否显示主窗口、是否检查小助手更新，以及文件夹批量导入是否递归。"),
