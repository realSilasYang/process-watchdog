...
라이트 및 다크 UI, 자동 복구, 실행 로그, 실행 취소와 다시 실행, 표시 이름과 아이콘 사용자 지정 기능을 제공합니다. Windows x64 배포 패키지에는 SPDX SBOM, SHA-256 체크섬, 빌드 출처 정보도 포함됩니다.
# 화면 개요
...
기본 창은 감시 대상 순서, 앱 아이콘, 이름, 권한 요구 사항, 현재 상태를 한곳에 보여 줍니다. 명령 모음에는 추가, 삭제, 일시 중지, 설정, 도움말, 정보가 있으며, 도움말에서 사용 설명서, 실행 로그, 피드백 페이지를 선택할 수 있습니다. 정보에는 버전, 실행 환경, 업데이트, 프로젝트, 후원 작업이 모여 있습니다. 아래쪽에는 실행, 복구, 일시 중지, 실패 대상 수가 표시되고, 비정상 상태의 판단 근거는 실행 로그에서 확인할 수 있습니다.
...
**[사용자 안내서](#사용자-안내서)**<br>
[설치](#1-설치와-첫-실행) · [항목 관리](#2-항목-추가와-관리) · [상태](#3-상태와-복구) · [설정](#4-설정) · [로그](#5-로그-진단-개인정보)
...
5. 로그에서 실제 사용한 식별 근거, 상태 확인, 복구 시도를 확인합니다.
...
목록에서 마우스 오른쪽 버튼을 눌러 파일 위치 열기, 대상 실행 종료, 대상 경로 편집, 프로세스 식별 및 실행 설정, 관리자 요구 사항 전환을 할 수 있습니다. 실행 종료는 감시도 일시 중지하여 대상이 자동으로 다시 시작되지 않게 합니다. 기본 창에만 적용되는 이름과 아이콘도 바꿀 수 있으며 대상 식별이나 시작 방식에는 영향을 주지 않습니다. 이미 기본 표시라면 기본값 복원 명령은 비활성화됩니다.
...
개인 설정은 실제 실행 디렉터리의 `watchdog.ini`에 저장합니다. 포터블판과 소스판은 각 진입 디렉터리를 사용합니다. 이 파일은 릴리스에 포함되거나 업데이트 때 덮어쓰이지 않습니다.
...
├─ src/                     구성, 핵심, 진단, 실행, 검사, 플랫폼, UI, 자동 업데이트
...
- 구성 스냅샷, 감시 대상은 같은 트랜잭션에서 반영하며 테스트가 개인 `watchdog.ini`를 읽거나 덮어쓰면 안 됩니다.
