...
<picture>
  ...
  <img src="images/process-watchdog-overview-light.png" alt="라이트 및 다크 테마의 프로세스 감시 도우미 기본 창" width="100%">
</picture>

기본 창은 감시 항목 순서, 앱 아이콘, 이름, 권한 요구 사항, 현재 상태를 한곳에 보여 줍니다. 위쪽 명령 모음에서 추가, 삭제, 일시 중지, 설정, 도움말 정보, 후원을 이용할 수 있으며 도움말 정보에서 사용 설명서나 실행 로그를 열 수 있습니다. 아래쪽에는 실행, 복구, 업데이트, 일시 중지, 실패 항목 수가 표시되고, 비정상 상태의 판단 근거는 실행 로그에서 확인할 수 있습니다.
...
- 관리자 권한 요구 사항을 항목별로 지정할 수 있습니다. 실행 중 인스턴스의 권한이 맞지 않으면 알리고, 수동 다시 시작은 설정에 따라 승격합니다.
...
- 중국어 간체, 중국어 번체(홍콩), 중국어 번체(대만), 영어, 일본어, 베트남어, 한국어, 스페인어, 프랑스어, 브라질 포르투갈어, 러시아어, 독일어, 이탈리아어를 지원합니다. 기본적으로 Windows 표시 언어를 따르고 지원하지 않는 언어는 영어로 돌아가며, 일반 설정에서 직접 선택할 수도 있습니다. 언어와 콘텐츠 글꼴 변경은 현재 프로세스에 즉시 적용되고 감시 작업을 중지하거나 다시 초기화하지 않습니다.
- “언어 기본값 따르기”는 PingFang, SF Pro Text, Harano Aji Gothic, Apple SD Gothic Neo를 우선 사용합니다. 장치에 없으면 상업적 배포 허가 또는 OFL이 적용된 번들 글꼴을 프로세스 전용으로 불러오고, 그래도 사용할 수 없으면 해당 Noto 글꼴로 대체합니다. 콘텐츠 글꼴은 본문, 입력란, 목록, 정보 페이지에 적용되며 버튼, 설정 탭, 기본 창 상태 표시줄은 현재 언어의 Windows 시스템 UI 굵은 글꼴을 사용합니다.
...
실제 장치에서 확인한 화면 배율은 현재 100%~200%입니다. 그 밖의 배율과 모니터 사이의 연속 DPI 전환은 코드만으로 검증되었다고 볼 수 없습니다. [호환성과 알려진 제한](en/compatibility.md)을 확인하세요.
...
프로세스 감시 도우미는 계속 오픈 소스로 유지됩니다. 장기적인 유지 관리는 여러분의 지원과 격려에 달려 있습니다. 문제를 찾거나 앱을 복구하는 시간을 줄이는 데 도움이 되었다면 아래 QR 코드로 자발적으로 후원할 수 있습니다. 후원금은 지속적인 유지 관리, 호환성 검증, 새 릴리스에 사용됩니다.
...
| 삭제 | 선택한 감시 항목 삭제. 다중 선택과 실행 취소 지원 |
| 일시 중지 / 재개 | 자동 감시만 전환하고 현재 실행 중인 대상은 종료하지 않음. 혼합 선택은 항목별 반전 |
| 설정 | 일반, 감시 및 시작, 중지 정책, 로그, 정보 설정 |
| 도움말 정보 | 내장 사용 설명서, 실행 로그 또는 GitHub 피드백 페이지 선택 |
| 후원 | 유지 관리를 돕는 WeChat Pay 및 Alipay QR 코드 표시 |
...
목록에서 마우스 오른쪽 버튼을 눌러 파일 위치 열기, 다시 시작, 대상 경로 편집, 프로세스 식별 및 실행 설정, 관리자 요구 사항 전환, 업데이트 보호 설정을 할 수 있습니다. 기본 창에만 적용되는 이름과 아이콘도 바꿀 수 있으며 대상 식별, 시작, 업데이트 보호에는 영향을 주지 않습니다. 이미 기본 표시라면 기본값 복원 명령은 비활성화됩니다.
...
| 분류 | 옵션 |
| --- | --- |
| 일반 | 바탕 화면/시작 메뉴 바로 가기, 예약 자동 시작, 두 가지 시작 동작, UI 언어, 콘텐츠 글꼴, 테마 |
| 감시 및 시작 | 프로세스 확인 간격, 충돌 후 자동 재시작 지연 순서, 폴더 가져오기 시 하위 폴더 포함 여부 |
| 중지 정책 | GUI/CLI 앱 종료 시간 제한과 초과 뒤 강제 종료 허용 여부 |
| 로그 | 시작할 때 지우기, 실행 로그 표시 한도, 일괄 로그 보존 일수, 저장 경로 |
| 정보 | 앱/실행 환경 버전, 지금 업데이트 확인, 오픈 소스 프로젝트 링크 |
...
- 구성 스냅샷, 감시 항목, 업데이트 보호 설정은 같은 트랜잭션에서 반영하며 테스트가 개인 `watchdog.ini`를 읽거나 덮어쓰면 안 됩니다.
