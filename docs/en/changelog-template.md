...
`docs/CHANGELOG.en.md`, then move released entries out of `[Unreleased]`. Keep only
categories that are actually present in the release.
...
- Use `## [X.Y.Z] - YYYY-MM-DD`. The version must match `VERSION`, the Ahk2Exe
  file version, and Git tag; build tooling uses this format for SBOM timestamps.
- Use Added, Changed, and Fixed for ordinary entries. Remove empty categories.
  Important notes comes first; Security appears only after coordinated disclosure.
