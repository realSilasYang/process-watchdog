...
<picture>
  ...
  <img src="images/process-watchdog-overview-light.png" alt="程序守護小助手的淺色與深色主介面" width="100%">
</picture>

主視窗集中顯示守護項目的順序、應用程式圖示、名稱、權限要求與目前狀態。頂端命令列提供新增、刪除、暫停、設定、說明資訊與贊助入口；說明資訊可再開啟使用說明或執行紀錄。底部狀態列彙整執行中、復原、更新、暫停與失敗數量，異常狀態可從執行紀錄追查實際判斷依據。
...
- 可要求以系統管理員身分執行；現有執行個體權限不符時會提示，手動重新啟動則依設定提升權限。
...
目前實體顯示縮放測試矩陣涵蓋 100% 到 200%；其他比例與跨螢幕連續 DPI 切換不能只憑程式碼視為已通過驗證。完整邊界請參閱[相容性與已知限制](en/compatibility.md)。
...
程序守護小助手會持續維持開放原始碼。專案的長期維護有賴您的支持與鼓勵；如果小助手替您節省了排查問題或復原程式的時間，歡迎透過下方 QR Code 自願贊助。您的支持會用於持續維護、相容性驗證與版本發行。
...
1. 從 [Releases](https://github.com/realSilasYang/process-watchdog/releases) 下載 `process-watchdog-<版本>-windows-x64.zip` 與相同版本的 `SHA256SUMS.txt`。
2. 驗證 ZIP、獨立 EXE 與 SBOM 的 SHA-256，然後完整解壓縮 ZIP。Release 另外保留的 EXE 只供封存及驗證；程式仍需要套件內的 DLL、資源與 `third_party` 目錄，不能只下載或複製 EXE 執行。
...
在主清單中按右鍵可開啟檔案位置、重新啟動、變更目標路徑，或設定處理程序辨識與啟動方式，也可切換管理員權限要求、設定軟體更新保護，以及自訂主視窗名稱和圖示。顯示自訂不會改變目標識別、啟動進入點或更新保護；目前已是預設顯示時，「還原預設」會停用。
...
| 執行中（權限不符） | 執行個體存在，但不符合該項目的系統管理員權限要求 |
...
個人設定儲存在程式目錄的 `watchdog.ini`，未完成的軟體更新工作階段儲存在 `watchdog.maintenance.ini`。兩者都由 Git 忽略，發行套件不會攜帶或覆寫；`config/watchdog.example.ini` 只說明目前預設值與欄位。

EXE 與原始碼都以自身進入點檔案所在目錄作為設定目錄：同一目錄時共用上述兩個檔案，不同目錄時各自獨立。全域單一執行個體鎖會阻止兩種形式同時執行。捷徑與排程工作會指向最後建立或切換整合的執行形式，因此每個安裝目錄應只選一個日常進入點。同目錄共存只適合暫時切換測試；需要兩套可獨立更新的安裝時，應放在不同目錄。詳情請參閱[設定、備份與復原](en/configuration.md)與[安裝、升級與移除](en/installation.md)。
...
- 設定快照、守護項目與更新保護設定在同一交易中提交；測試不可讀取或覆寫個人 `watchdog.ini`。
...
`verify.ps1` 檢查相依性雜湊、AHK 剖析、靜態架構限制、核心測試、儲存庫邊界、完整 Git 歷史洩漏、工作流程語法與啟動行為。`run-gui-tests.ps1` 建立真正的 Windows 控制項，涵蓋 13 種語言及字型即時切換、三層視窗與 GDI／USER 控制代碼回收。`reproducible-build.ps1` 連續建置兩次 EXE 套件、原始碼套件與 SBOM，並比較校驗清單雜湊。
