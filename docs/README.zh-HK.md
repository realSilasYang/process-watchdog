...
專案提供淺色與深色介面、自動恢復、執行記錄、復原與重做、自訂名稱及圖示，以及附有 SPDX SBOM、SHA-256 校驗和與建構溯源資料的 Windows x64 發行套件。
# 介面概覽
...
主視窗集中顯示守護對象的次序、應用程式圖示、名稱、權限要求及目前狀態。頂部指令列提供加入、刪除、暫停、設定、幫助資訊與捐贈入口；幫助資訊可再開啟使用說明或執行記錄。底部狀態列彙總執行中、恢復、暫停和失敗數量，異常狀態可從執行記錄追查具體判斷依據。
...
- 可要求以系統管理員身分執行；現有實例權限不符時會提示，下一次由守護啟動時會按設定提升權限。
- 設定以不可分割方式取代；無法解析的監控記錄會移至 `[Recovery]`，不會無提示遺失。
- 程式搜尋只使用 Everything 服務，不進行本機全碟掃描，也不限制結果數量；大量結果會分批加入，避免圖示擷取長時間佔用介面。
- 支援簡體中文、繁體中文（香港）、繁體中文（台灣）、英文、日文、越南文、韓文、西班牙文、法文、巴西葡萄牙文、俄文、德文和意大利文。預設跟隨 Windows 介面語言，不支援的語言會回退至英文，亦可在「顯示」手動切換。語言與內容字型儲存後即時生效，不會停止或重新初始化守護工作。
- 「跟隨語言預設」只使用 Windows 已安裝字型，依次嘗試苹方、SF Pro Text、Harano Aji Gothic 或 Apple SD Gothic Neo、對應的 Noto 字型及 Windows 系統字型。可選字型套件須先安裝到 Windows；小助手不會從自身目錄私有載入字型。內容字型適用於正文、輸入欄、清單及「關於」資訊；按鈕、設定分頁及主視窗底部狀態列則固定使用目前語言對應的 Windows 系統 UI 粗體字型。
...
**[使用指南](#使用指南)**<br>
[安裝與首次執行](#1-安裝與首次執行) · [加入與管理項目](#2-加入與管理項目) · [狀態與恢復](#3-狀態與恢復) · [設定](#4-設定) · [記錄與診斷](#5-記錄診斷與私隱)
...
如果小助手為您節省了排查故障或恢復程式的時間，歡迎透過下方二維碼打賞作者。請選擇扶貧方式：
...
1. 從 [Releases](https://github.com/realSilasYang/process-watchdog/releases) 選擇完整可攜式 ZIP 或完整原始碼 ZIP；可選字型包不是第三個程式版本。
2. 可攜式 ZIP 在完整解壓的目錄執行且無需另裝 AutoHotkey；原始碼 ZIP 需要 AutoHotkey v2 x64。字型包需安裝到 Windows 且不是執行必需；程式搜尋另需 [Everything 官方最新版](https://www.voidtools.com/downloads/)。
3. 執行 `进程守护小助手.exe`。程式會要求系統管理員權限，並按設定顯示主視窗或靜默留在系統匣。
4. 選擇「加入」以選取目標，亦可把支援的檔案拖入主視窗。
5. 開啟「記錄」，查看小助手實際採用的身分證據、狀態檢查、恢復嘗試。
...
「小助手已是最新版本」與「本機 AutoHotkey 已是最新版本」是兩件事。每次正式發佈開始時，流程會選取最新穩定版 AutoHotkey 及最新發佈版 Ahk2Exe，凍結版本並完成全部測試後，才把所選 AutoHotkey 封裝至 EXE。主視窗「關於」會同時顯示小助手版本、EXE／原始碼形態及實際 AutoHotkey 版本，亦可手動檢查更新。詳情請參閱[版本、執行形態與更新責任](en/versioning.md)。
...
| 刪除 | 移除選取的守護對象；支援多選，亦可復原 |
| 暫停／恢復 | 只改變自動守護狀態，不會關閉目前正在執行的目標；混合選取時逐項切換 |
| 設定 | 設定顯示、啟動、監控、停止策略及記錄選項 |
| 幫助 | 選擇內置使用說明、執行記錄或 GitHub 意見回報頁面 |
| 關於 | 查看軟件版本及執行環境、檢查更新、開啟開源地址或進入打賞視窗 |

加入項目時可設定啟動入口、工作目錄、參數及是否要求系統管理員權限。LNK 會保留為啟動入口，實際程式路徑則獨立用於程序識別，因此安裝程式建立的間接捷徑無需手動改成容易變動的內部 EXE。

在主清單中按右鍵可開啟檔案位置、結束目標執行、更改目標路徑，或設定程序識別與啟動方式，也可切換管理員權限要求，以及自訂主視窗名稱和圖示。「結束執行」亦會暫停守護，避免目標被自動重新啟動。顯示自訂不會改變目標識別或啟動入口；目前已是預設顯示時，「恢復預設」會停用。
...
| --- | --- |
| 執行中 | 已找到符合目標身分的執行中實例 |
| 執行中（權限不符） | 實例存在，但不符合該守護對象的系統管理員權限要求 |
...
| --- | --- |
| 顯示 | 介面語言、介面內容字型及主題 |
| 啟動 | 桌面及開始功能表捷徑、排程自動啟動，以及兩項啟動行為 |
| 監控 | 程序狀態檢查間隔、意外結束後的自動重新啟動延遲序列，以及匯入資料夾時是否包含子資料夾 |
...
個人設定儲存在實際執行目錄的 `watchdog.ini`。可攜式與原始碼版本使用各自入口目錄。此檔案由 Git 忽略，發行套件不會攜帶或覆寫。
...
├─ src/                     設定、核心、診斷、執行、探測、平台、UI 及自動更新
...
- 設定快照、守護對象在同一交易中提交；測試不可讀取或覆寫個人 `watchdog.ini`。
