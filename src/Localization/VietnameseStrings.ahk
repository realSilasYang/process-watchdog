    ...
    static Create() {
        ...
        catalog.Set(
            "• “重新启动”会先请求目标正常退出；超过设置时间后，是否强制终止由“停止”中的选项决定。",
                "• “Khởi động lại” trước hết yêu cầu đích thoát bình thường`; sau khi quá thời gian đã đặt, tùy chọn trong “Dừng” quyết định có buộc kết thúc hay không.")
        catalog.Set(
        ...
        catalog.Set(
            "手动触发了重新启动：{1}",
                "Đã yêu cầu khởi động lại thủ công: {1}")
        catalog.Set(
            "手动重启已取消，原进程未能停止：{1}",
                "Đã hủy khởi động lại thủ công vì không thể dừng tiến trình cũ: {1}")
        catalog.Set(
            "托管窗口生命周期尚未配置",
        ...
        catalog.Set(
            "暂时无法重新启动",
                "Tạm thời không thể khởi động lại")
        catalog.Set(
        ...
        catalog.Set(
            "创建桌面快捷方式，并将小助手加入开始菜单“所有”列表；是否固定到开始菜单由您决定。",
                "Tạo lối tắt trên màn hình nền và thêm trợ lý vào danh sách Tất cả ứng dụng trong menu Bắt đầu. Bạn có thể tự chọn có ghim vào menu Bắt đầu hay không.")
        catalog.Set(
            "创建成功！",
                "Đã tạo!")
        catalog.Set(
        ...
        catalog.Set(
            "该软件正在升级保护中。请等待升级完成，或在“软件升级保护”中结束等待后再重新启动。",
                "Phần mềm này đang được bảo vệ khi cập nhật. Hãy đợi cập nhật hoàn tất hoặc kết thúc chờ trong “Bảo vệ khi cập nhật” rồi mới khởi động lại.")
        catalog.Set(
            "语义版本无效",
        ...
        catalog.Set(
            "• “重新启动”会先请求目标正常退出；超过设置时间后，是否强制终止由“停止策略”中的选项决定。",
                "• “Khởi động lại” trước tiên sẽ yêu cầu ứng dụng đích thoát bình thường. Khi hết thời gian chờ, tùy chọn trong “Chính sách dừng” sẽ quyết định có buộc ứng dụng kết thúc hay không.")
        catalog.Set(
        ...
        catalog.Set("支持开源项目`n可使用微信支付或支付宝扫码捐赠", "Ủng hộ dự án mã nguồn mở`nQuét mã bằng WeChat Pay hoặc Alipay")
        catalog.Set("帮助信息", "Trợ giúp")
        catalog.Set("提交反馈", "Gửi phản hồi")
        catalog.Set("支持开源项目", "Ủng hộ dự án mã nguồn mở")
        catalog.Set("如果小助手为您节省了排查问题和恢复程序的时间，欢迎通过下方二维码打赏作者！`n进程守护小助手持续保持开源，项目的长期维护有赖于您的支持和鼓励~", "Nếu trợ lý đã giúp bạn tiết kiệm thời gian tìm nguyên nhân sự cố và khôi phục chương trình, hãy ủng hộ tác giả qua mã QR bên dưới!`nTrợ lý giám sát tiến trình sẽ tiếp tục là phần mềm mã nguồn mở; việc duy trì dự án lâu dài cần sự ủng hộ và động viên của bạn.")
        ...
        catalog.Set("守护目标更名识别服务未能启动。", "Không thể khởi động dịch vụ nhận diện đích giám sát được đổi tên.")
        catalog.Set("检测到守护目标可能已更名，等待用户确认：{1} -> {2}", "Đích giám sát có thể đã được đổi tên`; đang chờ người dùng xác nhận: {1} -> {2}")
        catalog.Set("守护目标更名识别异常：{1}", "Lỗi nhận diện đích giám sát được đổi tên: {1}")
        catalog.Set("确认窗口暂时无法显示，将稍后重试", "Tạm thời không thể hiển thị cửa sổ xác nhận. Sẽ thử lại sau.")
        catalog.Set("守护目标目录监听异常（{1}）：{2}", "Lỗi theo dõi thư mục của đích giám sát（{1}）: {2}")
        ...
        catalog.Set("小助手找到了与原文件身份一致的新路径。确认后将更新守护目标，名称、图标和启动设置保持不变。", "Trợ lý đã tìm thấy đường dẫn mới có cùng định danh tệp với tệp ban đầu. Sau khi xác nhận, đích giám sát sẽ được cập nhật mà vẫn giữ nguyên tên, biểu tượng và thiết lập khởi chạy.")
        catalog.Set("原路径：", "Đường dẫn cũ:")
        catalog.Set("新路径：", "Đường dẫn mới:")
        catalog.Set("Windows 文件身份一致", "Định danh tệp Windows trùng khớp")
        catalog.Set("Windows 重命名事件", "Sự kiện đổi tên của Windows")
