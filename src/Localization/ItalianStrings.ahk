...
class ItalianStrings {
    static Create() {
        ...
        catalog.Set(
            "已更新应用程序路径。",
                "Il percorso dell'applicazione è stato aggiornato.")
        catalog.Set(
            "已更新软件升级保护设置：{1}",
        ...
        catalog.Set(
            "拒绝将应用路径改为已存在的监控项：{1}",
                "Modifica del percorso dell'applicazione rifiutata perché esiste già un elemento di monitoraggio uguale: {1}")
        catalog.Set(
            "按钮绘制器",
        ...
        catalog.Set(
            "项目名称：{1}`n真实路径：{2}",
                "Nome dell'elemento: {1}`nPercorso reale: {2}")
        catalog.Set(
            "🌿 环境变量：{1} 项`n",
        ...
        catalog.Set("支持开源项目`n可使用微信支付或支付宝扫码捐赠", "Sostieni il progetto open source`nDona con WeChat Pay o Alipay")
        catalog.Set("帮助信息", "Aiuto")
        catalog.Set("提交反馈", "Invia feedback")
        catalog.Set("支持开源项目", "Sostieni il progetto open source")
        catalog.Set("如果小助手为您节省了排查问题和恢复程序的时间，欢迎通过下方二维码打赏作者！`n进程守护小助手持续保持开源，项目的长期维护有赖于您的支持和鼓励~", "Se l’assistente ti ha fatto risparmiare tempo nella diagnosi dei problemi e nel ripristino dei programmi, puoi sostenere l’autore tramite i codici QR qui sotto!`nProcess Watchdog resterà sempre open source e la manutenzione a lungo termine del progetto dipende dal tuo sostegno e incoraggiamento.")
        ...
        catalog.Set("该项目直接启动并监控同一个目标，无需额外识别真实进程。", "Questo elemento avvia e monitora direttamente la stessa destinazione, quindi non occorre identificare separatamente il processo effettivo.")
        ...
        catalog.Set("• 右键项目可自定义主窗口名称和图标，也可打开所在位置、重新启动、编辑路径、切换管理员运行、配置进程识别与启动设置及软件升级保护，并查看批处理输出日志。要求管理员运行但当前权限不符时会显示警告；右键重新启动会按该设置提权启动。", "• Fare clic con il pulsante destro su un elemento per personalizzarne nome e icona nella finestra principale, aprire il percorso, riavviarlo, modificarne il percorso, attivare o disattivare l'avvio come amministratore, configurare identificazione del processo, avvio e protezione degli aggiornamenti oppure consultare il registro di output dei file batch. Se è richiesto l'avvio come amministratore ma il processo corrente non dispone dei privilegi necessari, verrà mostrato un avviso; riavviando dal menu contestuale il programma verrà elevato secondo questa impostazione.")
        ...
        catalog.Set("; AppN 与 [Apps] 中同名项目一一对应，依次保存启动程序或解释器路径及其参数。", "; Ogni AppN corrisponde all'elemento omonimo in [Apps] e memorizza, nell'ordine, il percorso del launcher o dell'interprete e i relativi argomenti.")
        ...
        catalog.Set("监控项不能指向文件夹：{1}", "Un elemento sorvegliato non può puntare a una cartella: {1}")
        ...
        catalog.Set("• 确认后只更新守护路径，名称、图标和启动设置保持不变；关闭小助手期间发生的更名或跨磁盘移动无法可靠自动识别，请双击项目手动更新完整路径。", "• Dopo la conferma viene aggiornato solo il percorso monitorato. Il nome, l'icona e le impostazioni di avvio restano invariati. Le rinomine effettuate mentre l'assistente è chiuso e gli spostamenti su un altro volume non possono essere rilevati automaticamente in modo affidabile. In questi casi, fare doppio clic sull'elemento e aggiornare manualmente il percorso completo.")
        return catalog
    }
}
