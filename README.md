...
项目提供浅色与深色图形界面、自动恢复、运行日志、撤销与重做、自定义名称和图标，以及包含 SPDX SBOM、SHA-256 校验和与构建溯源的 Windows x64 发行包。
...
主窗口集中展示守护对象顺序、应用图标、名称、权限要求和当前状态。顶部命令栏提供添加、删除、暂停、设置、帮助与关于入口；帮助中可继续选择使用说明、运行日志或提交反馈，关于窗口集中显示版本、运行环境、更新、开源地址和打赏入口。窗口底部汇总运行、恢复、暂停和失败数量，异常状态可以继续通过运行日志追踪到具体判断依据。

## 主要能力

- 守护 EXE、AHK、Python、JavaScript、PowerShell、BAT、CMD 和 LNK。
- 使用 `Running`、`Stopped`、`Unknown` 三态探活；未知状态不会触发盲目重启。
- 主列表序号支持七种主题配色彩点；右键已选条目可批量设置圆点颜色，用于视觉分组或优先级标识。颜色只影响显示，不参与进程识别或启动。
- 每个目标拥有独立控制器、代际和任务令牌，暂停、删除或改路径后旧回调立即失效。
- 直接文件或其上级目录被更名、跨目录或跨磁盘移动后，会先按扩展名和精确大小高效筛选，再以完整 SHA-256 内容哈希确认新路径并由用户确认；即使移动发生在小助手关闭期间也能识别。内容迁移不使用文件名、文件 ID 或目录监听。
...
**[用户使用指南](#用户使用指南)**<br>
[安装与首次运行](#1-安装与首次运行) · [添加和管理守护对象](#2-添加和管理守护对象) · [状态与恢复逻辑](#3-状态与恢复逻辑) · [设置](#4-设置) · [日志诊断和隐私](#5-日志诊断和隐私)
...
5. 从“帮助 → 运行日志”查看目标识别、状态检查和恢复重试实际采用的证据。
...
| 监控 | 进程状态检查间隔、崩溃自动重启延迟序列、开启停止后询问恢复时的起始停止次数和导入文件夹时是否包含子目录 |
...
个人配置保存在实际运行目录的 `watchdog.ini`。便携版和源码版都以各自入口目录为实际运行目录；该文件被 Git 忽略，发行包不会携带或覆盖。仓库中的 `config/watchdog.example.ini` 只用于说明当前默认值和字段。
...
- 配置快照和守护对象在同一事务中提交，测试不得读取或覆盖正式 `watchdog.ini`。
