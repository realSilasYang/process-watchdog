...
显示缩放的目标手工矩阵目前覆盖 100%～200%；其他比例和跨显示器连续 DPI 切换不能仅凭代码推定为已经通过。完整边界见[兼容性与已知限制](docs/compatibility.md)。
...
添加项目时可以配置启动入口、工作目录、参数和是否要求管理员权限。LNK 始终保留为启动入口，真实程序路径单独用于进程识别，因此安装器生成的间接快捷方式也不需要手工改成某个容易变化的内部 EXE。
